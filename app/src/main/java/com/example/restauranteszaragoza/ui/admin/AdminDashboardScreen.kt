    ...
    LaunchedEffect(Unit) {
        scope.launch {
            try {
                ...
                stats = EstadisticasAdmin(
                    totalUsuarios = 12, totalRestaurantes = 8,
                    totalReservas = 47, reservasHoy = 5, ingresosPropinas = 138.50
                )
            }
        }
    }
                ...
                Box(
                    ...
                    Row(Modifier.fillMaxWidth(), Arrangement.SpaceBetween, Alignment.CenterVertically) {
                        Column {
                            ...
                            Text(
                                "Bienvenido, ${SessionManager.usuarioActual?.nombre ?: "Admin"}",
                    ...
                    tabs.forEachIndexed { i, label ->
                        Tab(selected = tabIndex == i, onClick = { tabIndex = i }) {
                            Text(label, modifier = Modifier.padding(10.dp),
                                color = if (tabIndex == i) ACCENT else Color.Gray, fontSize = 13.sp)
                        }
                    }
                }
                        ...
                        items(usuarios) { u ->
                            AdminUsuarioCard(
                                ...
                                onToggle = {
                                    scope.launch {
                                        try {
                                            RetrofitClient.instancia.toggleUsuario(
                                                mapOf("usuario_id" to u.id.toString())
                                            )
                                            usuarios = RetrofitClient.instancia.listarUsuarios()
                                            snackMsg = "Estado de usuario actualizado"
                        ...
                        items(restaurantes) { r ->
                            AdminRestauranteCard(
                                ...
                                onToggle = {
                                    scope.launch {
                                        try {
                                            RetrofitClient.instancia.toggleRestaurante(
                                                mapOf("restaurante_id" to r.id.toString())
                                            )
                                            restaurantes = RetrofitClient.instancia.listarRestaurantesAdmin()
...
// ─────────────────────────────────────────────────────────────────────────────
// Card de Usuario (admin)
// ─────────────────────────────────────────────────────────────────────────────
@Composable
private fun AdminUsuarioCard(usuario: Usuario, onToggle: () -> Unit) {
        ...
        Row(
            ...
            Box(Modifier.size(46.dp).background(rolColor.copy(0.15f), CircleShape), Alignment.Center) {
                Text(
                    ...
                    color = rolColor, fontWeight = FontWeight.Bold, fontSize = 18.sp
                )
            }
            Spacer(Modifier.width(12.dp))
            Column(Modifier.weight(1f)) {
                ...
                Surface(color = rolColor.copy(0.15f), shape = RoundedCornerShape(8.dp)) {
                    Text(
                        usuario.rol.uppercase(), color = rolColor, fontSize = 10.sp,
            ...
            IconButton(onClick = onToggle) {
                Icon(Icons.Default.ToggleOn, "Toggle", tint = ACCENT, modifier = Modifier.size(28.dp))
            }
        }
    }
}
...
// ─────────────────────────────────────────────────────────────────────────────
// Card de Restaurante (admin)
// ─────────────────────────────────────────────────────────────────────────────
@Composable
private fun AdminRestauranteCard(restaurante: Restaurante, onToggle: () -> Unit) {
    // activo: en el modelo es Int (1=activo, 0=inactivo)
        ...
        Row(
            ...
            Column(Modifier.weight(1f)) {
                ...
                Row(
                    ...
                    Surface(color = estadoColor.copy(0.15f), shape = RoundedCornerShape(6.dp)) {
                        Text(
                            if (isActivo) "ACTIVO" else "INACTIVO",
                            color = estadoColor, fontSize = 10.sp, fontWeight = FontWeight.Bold,
            ...
            IconButton(onClick = onToggle) {
                Icon(
                    ...
                    "Toggle",
                    tint = if (isActivo) ACCENT else Color.Gray,
    ...
    Card(shape = RoundedCornerShape(16.dp), colors = CardDefaults.cardColors(containerColor = CARD_BG)) {
        Row(
            ...
            Column(Modifier.weight(1f)) {
                ...
                Text(
                    "${reserva.nombreUsuario.ifBlank { "Usuario #${reserva.usuarioId}" }} · ${reserva.numPersonas} pax",
